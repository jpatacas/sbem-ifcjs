...
function createSimpleChild(parent, node) {
    ...
    childNode.onclick = async () => {
        const res = viewer.IFC.selector.pickIfcItemsByID(0, [node.expressID]);
        //console.log(res);
        //display properties
        //console.log(viewer.IFC.selector.pickIfcItemsByID(0, [node.expressID]));
       // const result = await viewer.IFC.selector.pickIfcItem(); //highlightIfcItem hides all other elements
        if (!result) return;
        const { modelID, id } = result;
        const props = await viewer.IFC.getProperties(0, id, true, false); //can also use getTypeProperties(), getMaterialProperties() and getPropertySets()
        console.log(props); //call the function here
        createPropertiesMenu(props);
    }
}
